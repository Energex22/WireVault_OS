...
import { createHomeView } from './views/home-view.js?v=1.6.2';
...
const controllerStatusSummary =
  document.getElementById('controllerStatusSummary');
const controllerCountBadge =
  document.getElementById('controllerCountBadge');
const controllerDeviceList =
  document.getElementById('controllerDeviceList');

function controllerBattery(gamepad) {
  const raw =
    gamepad?.batteryLevel ??
    gamepad?.battery?.level ??
    gamepad?.battery ??
    gamepad?.buttons?.battery;
...
function controllerDisplayName(gamepad, index) {
  const id = gamepad?.id?.trim();
  if (!id) return `Controller ${index + 1}`;
...
function renderControllerStatus() {
  if (!controllerDeviceList) return;

  const gamepads = Array.from(navigator.getGamepads?.() || [])
    .filter(gamepad => gamepad && gamepad.connected);

  controllerCountBadge.textContent = String(gamepads.length);
  controllerStatusSummary.textContent = gamepads.length
    ? `${gamepads.length} connected`
    : 'None connected';

  controllerDeviceList.replaceChildren();

  if (!gamepads.length) {
    const empty = document.createElement('div');
    empty.className = 'controller-empty-state';
    empty.innerHTML =
      '<span>🎮</span><div><strong>No controllers connected</strong>' +
      '<small>Connect a controller to view its status.</small></div>';
    controllerDeviceList.append(empty);
    return;
  }

  gamepads.forEach((gamepad, index) => {
    const battery = controllerBattery(gamepad);
    const row = document.createElement('div');
    row.className = 'controller-device-row';

    const icon = document.createElement('span');
    icon.className = 'controller-device-icon';
    icon.textContent = '🎮';

    const copy = document.createElement('div');
    copy.className = 'controller-device-copy';

    const name = document.createElement('strong');
    name.textContent = controllerDisplayName(gamepad, index);

    const detail = document.createElement('small');
    detail.textContent =
      `${gamepad.mapping || 'standard'} mapping · ${gamepad.buttons.length} buttons`;

    copy.append(name, detail);

    const batteryWrap = document.createElement('div');
    batteryWrap.className =
      `controller-battery ${battery === null ? 'unavailable' : ''}`;

    const batteryGlyph = document.createElement('span');
    batteryGlyph.textContent = batteryIcon(battery);

    const batteryText = document.createElement('small');
    batteryText.textContent =
      battery === null ? 'Battery unavailable' : `${battery}%`;

    batteryWrap.append(batteryGlyph, batteryText);
    row.append(icon, copy, batteryWrap);
    controllerDeviceList.append(row);
  });
}

window.addEventListener('gamepadconnected', event => {
  renderControllerStatus();
  window.WireVault?.toast?.(
    `${controllerDisplayName(event.gamepad, event.gamepad.index)} connected`
  );
});

window.addEventListener('gamepaddisconnected', () => {
  renderControllerStatus();
  window.WireVault?.toast?.('Controller disconnected');
});

setInterval(renderControllerStatus, 2000);
renderControllerStatus();
