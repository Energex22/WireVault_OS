...
export function createHomeView({ store, bus, coreApi, library, router }) {
  const wrapper = el('div',{
    'data-wirevault-home-version':'1.6.2'
  });

const legacyLayoutKey = 'wirevault.home.layout.v1';
const defaultOrder = [
...
function activeProfileId() {
  return (
    window.WireVaultProfiles?.active?.id ||
    localStorage.getItem('wirevault.profile.active') ||
    'default'
  );
}

function layoutKey() {
  return `${legacyLayoutKey}.${activeProfileId()}`;
}

function migrateLegacyLayout() {
  const profileKey = layoutKey();
...
function decorateWidget(node, id) {
  ...
  node.classList.toggle('widget-layout-wide', isWide);
  node.classList.toggle('widget-editing', editMode);
  node.draggable = false;

  if (!editMode) return node;

  const controlButton = ({ label, title, action, className = '' }) =>
    el('button',{
      type:'button',
  ...
  const controls = el('div',{className:'widget-edit-controls'},[
    controlButton({
      label:'←',
      title:'Move earlier',
      action:() => moveWidget(id,-1)
    }),
    controlButton({
      label:'→',
      title:'Move later',
    ...
    controlButton({
      label:isWide ? '1×' : '2×',
      title:isWide ? 'Use standard width' : 'Use wide width',
      action:() => toggleWide(id)
    }),
    controlButton({
      label:'×',
      title:'Hide widget',
      className:'widget-hide-control',
      action:() => hideWidget(id)
    })
  ]);
  ...
  bus.on('library:refreshed',() => { if (!editMode) render(); });
  bus.on('core:online',() => { if (!editMode) refreshDashboard(); });
  bus.on('core:offline',() => { if (!editMode) render(); });
  bus.on('core:library.scan.complete',() => { if (!editMode) refreshDashboard(); });
